...
import { Gavel, User, Mail, Fingerprint, Lock, Loader2, Phone } from "lucide-react";
...
export default function RegisterForm({ 
  ...
  role: "lawyer" | "client",
  // CHANGED: Added 'email' parameter to match the Navbar's expectation
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.status === "success") {
        ...
        // CHANGED: Pass the normalizedEmail back to the parent (Navbar)
        // This triggers the switch to Step 3 (OTP) inside the dialog
        if (onSuccess) {
          onSuccess(normalizedEmail);
        } else {
          // Fallback if this form is ever used as a standalone page
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-2">
          <Label htmlFor="fullName" className="text-xs uppercase tracking-wider font-bold text-muted-foreground">Full Name</Label>
      ...
      <div className="space-y-2">
        <Label htmlFor="identification" className="text-xs uppercase tracking-wider font-bold text-muted-foreground">
          {role === "lawyer" ? "Bar Council ID" : "National ID / Aadhaar"}
        </Label>
        <div className="relative">
          {role === "lawyer" ? (
          ...
          <Input 
            id="identification" 
            name="identification" 
            placeholder={role === "lawyer" ? "WB/1234/2024" : "Enter ID Number"} 
            className="pl-10 h-11 border-border focus-visible:ring-primary" 
            required 
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          <span className="absolute left-10 top-3 text-sm font-medium text-muted-foreground">+91</span>
          <Input 
            id="phone" 
            name="phone" 
            type="tel" 
